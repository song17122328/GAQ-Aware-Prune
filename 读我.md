```python
# 3. 运行结构化均匀剪枝（约3分钟+3小时后训练）
python llama3.py --pruning_ratio 0.25 \
    --base_model /newdata/LLMs/Llama-3-8B-Instruct \
    --block_wise \
    --block_mlp_layer_start 3 --block_mlp_layer_end 30 \
    --block_attention_layer_start 3 --block_attention_layer_end 30 \
    --pruner_type taylor \
    --save_model
```


🚀 现在可以重新运行
# 拉取最新代码
git pull origin claude/fix-tuple-grad-fn-error-011CUywXCqgkBWjHYGzNQQJN

# 运行剪枝（所有层）
python llama3_unbalanced_pruning.py \
    --base_model /newdata/LLMs/Llama-3-8B-Instruct \
    --save_ckpt_log_name "llama_unbalanced_prune_all_layers" \
    --pruning_ratio 0.25 \
    --importance_method removal \
    --pruning_strategy inverse \
    --alpha 1.5 \
    --block_attention_layer_start 0 \
    --block_attention_layer_end 32 \
    --block_mlp_layer_start 0 \
    --block_mlp_layer_end 32 \
    --save_model \
    --test_after_train



python check_model_size.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama_unbalanced_prune_all_layers/pytorch_model.bin

#测试
python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama_unbalanced_prune_all_layers/pytorch_model.bin

python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama_prune/pytorch_model.bin

python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/test_uniform_prune/pytorch_model.bin

python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama_unbalanced_prune_v2/pytorch_model.bin

# 检测最新的GQA感知的剪枝情况
python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama3_gqa_aware_pruned_v3/pytorch_model.bin

# 检测微调的GQA感知的剪枝情况
python analyze_pruning.py \
--original_model /newdata/LLMs/Llama-3-8B-Instruct \
--pruned_model prune_log/llama3_gqa_aware_pruned_v3/pytorch_model.bin