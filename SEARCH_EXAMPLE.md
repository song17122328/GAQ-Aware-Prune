...
本脚本采用**智能双向搜索 + 早停**策略，支持**两阶段或三阶段贪心搜索**，相比传统的线性搜索，可节省30-50%的测试时间。

### 搜索策略

#### 两阶段搜索（默认）
1. **阶段1：粗粒度分布搜索**（步长=1）
2. **阶段2：细粒度分布搜索**（步长=0.1）

#### 三阶段搜索（启用 `--search_freeze_layers`）
1. **阶段1：粗粒度分布搜索**（步长=1）
2. **阶段2：细粒度分布搜索**（步长=0.1）
3. **阶段3：冻结层数搜索**（在最优分布下贪心搜索）

### 核心特性

1. **智能双向搜索**
   - 从智能起点开始（默认2:8，基于LLaMA-3实际Attention:MLP参数比例）
   - 向两边同时搜索，避免盲目测试所有比例
...
3. **两阶段贪心（冻结层优化）**
   - 先固定freeze=0，搜索最优分布
   - 在最优分布下，搜索最优冻结层数
   - 避免exhaustive grid search（分布×冻结层=120+次测试）
   - 两阶段贪心：~23次测试即可完成

4. **搜索效率**
   - 粗粒度：从11次减少到5-9次（节省20-50%）
   - 细粒度：自动停止远离最优点的测试
   - 冻结层搜索：8个候选值，通常测试4-6个（早停）
   - 总体：预计节省30-40%时间
...
**阶段1：粗粒度搜索**（基于您的数据）：
```
起点: 2:8 (智能起点，基于LLaMA-3实际参数比例 19.2%:80.8%)
向左: 1:9 (PPL = 73.59) → 0:10 (PPL = 46.87) ✅最优
向右: 3:7 (PPL = 137.70) → 4:6 (PPL更高，加速增大，早停！)
      [跳过 5:5, 6:4, 7:3, 8:2, 9:1, 10:0]
节省: 6次测试（55%）
```
...
向右: 0.1:9.9 → 0.2:9.8 → 0.3:9.7 (PPL = 45.12) ✅最优
      0.4:9.6 → 0.5:9.5 → 0.6:9.4 (PPL加速增大，早停！)
      [跳过 0.7:9.3, 0.8:9.2, 0.9:9.1, 1.0:9.0]
节省: 4次测试（40%）
```

**阶段3：冻结层数搜索**（可选，在最优分布0.3:9.7下）：
```
搜索范围: [0, 1, 2, 3, 4, 5, 6, 8]
freeze=0: PPL = 45.12 (基准)
freeze=1: PPL = 44.98 ✅
freeze=2: PPL = 44.85 ✅最优
freeze=3: PPL = 45.01
freeze=4: PPL = 45.23 (连续增大，早停！)
      [跳过 freeze=5, 6, 8]
节省: 3次测试（38%）
最优配置: 0.3:9.7 + freeze=2
```
...
**说明**：
- 默认起点：2:8（基于LLaMA-3的Attention:MLP参数比例）
- 可根据具体模型架构调整起点
- 更接近最优点的起点可以进一步提升搜索效率
...
**预期结果**：
- 阶段1+2：找到最优分布（例如 0.3:9.7）
- 阶段3：在该分布下找到最优冻结层数（例如 freeze=2）
- 最终输出完整的最优配置命令
