...
我们提供了自动化搜索脚本 `search_optimal_distribution.py`，采用**智能双向搜索+早停**策略自动找到使 PPL 最小的剪枝分布：
...
```
============================================================
阶段1: 智能粗粒度搜索 (步长=1, 带早停)
============================================================

从中心点开始: 5:5
✅ 实验完成: 5.0:5.0 -> PPL = 142.35

向左搜索 (减少Attention比例):
  测试 4:6
  ✅ 实验完成: 4.0:6.0 -> PPL = 125.42
  测试 3:7
  ✅ 实验完成: 3.0:7.0 -> PPL = 137.70
  测试 2:8
  ✅ 实验完成: 2.0:8.0 -> PPL = 83.77
  测试 1:9
...
向右搜索 (增加Attention比例):
  测试 6:4
  ✅ 实验完成: 6.0:4.0 -> PPL = 156.23
  测试 7:3
  ✅ 实验完成: 7.0:3.0 -> PPL = 178.91
  测试 8:2
  ✅ 实验完成: 8.0:2.0 -> PPL = 215.67
  ⚠️  检测到PPL持续增大且加速，停止向右搜索
     最近3次PPL: [142.35, 156.23, 178.91]
...
向右精细搜索 (增加Attention):
  ...
  测试 0.5:9.5
  ✅ 实验完成: 0.5:9.5 -> PPL = 52.34
  测试 0.6:9.4
  ✅ 实验完成: 0.6:9.4 -> PPL = 58.91
  ⚠️  检测到PPL持续增大且加速，停止向右搜索
     最近3次PPL: [47.23, 52.34, 58.91]
